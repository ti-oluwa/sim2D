...
```python
def run(
    input: Union[BlackOilFluid[ThreeDimensions], Run],
...
- A `BlackOilFluid[ThreeDimensions]` - the static model you built. When you pass a `BlackOilFluid`, you must also supply `config`.
...
- Required when `input` is a `BlackOilFluid`.
...
model = bores.BlackOilFluid.from_file("model.h5")
...
```python
def monitor(
    input: Union[BlackOilFluid, Run, Iterable[ModelState]],
    config: Optional[Config] = None,
    *,
