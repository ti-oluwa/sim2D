...
class RockCompressibilityRegions(StoreSerializable):
    ...
    and `load_compressibility_regions`).
    """
    ...
    def __init__(
        self,
        regions: typing.Dict[int, RockCompressibilityTable],
    ) -> None:
        """
        Build a `RockCompressibilityRegions` from a pre-built regions dict.

        :param regions: Mapping from 1-based `ROCKNUM` index to table.
    ...
    def for_region(self, rocknum: int) -> RockCompressibilityTable:
        ...
        """
        table = self.regions.get(rocknum)
        if table is None:
    ...
    def from_deck_file(
        ...
        *,
        ...
        """
        regions = load_compressibility_regions(
            deck_file=deck_file,
            interpolation_method=interpolation_method,
            dtype=dtype,
        )
    ...
    def __dump__(self, recurse: bool = True) -> typing.Dict[str, typing.Any]:
        return {
            "regions": {
                ...
                for rocknum, table in self.regions.items()
            }
        }
    ...
    def __load__(cls, data: typing.Mapping[str, typing.Any]) -> Self:
        regions = {
            ...
            for rocknum, table_data in data["regions"].items()
        }
        return cls(regions=regions)

    def __repr__(self) -> str:
        region_keys = sorted(self.regions.keys())
        return (
            f"{self.__class__.__name__}("
...
def load_compressibility_regions(
    deck_file: DeckFile,
    *,
    interpolation_method: InterpolationMethod = "linear",
    dtype: typing.Optional[npt.DTypeLike] = None,
    ...
    for region_idx in range(n_regions):
        ...
        if rocktab_all is not None and region_idx < len(rocktab_all):
            ...
            if records:
                regions[rocknum] = RockCompressibilityTable.from_rocktab_records(
                    rocktab_records=records,
                    interpolation_method=interpolation_method,
                    unit_system=unit_system,
                    dtype=dtype,
        ...
        if rock_all is not None and region_idx < len(rock_all):
            ...
            if region_rows:
                record = region_rows[0]
                regions[rocknum] = RockCompressibilityTable.from_rock_record(
                    reference_pressure=float(record["p_ref"]),
                    rock_compressibility=float(record["cr"]),
                    interpolation_method=interpolation_method,
                    unit_system=unit_system,
