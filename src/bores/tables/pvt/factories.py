...
def build_oil_pvt_data(
    ...
    **kwargs: typing.Any,
    ...
    :param specific_gravity_table: Pre-computed γo(P, T) (n_p, n_t).
    :param molecular_weight_table: Pre-computed Mo(P, T) (n_p, n_t) in lbm/lb-mol.
    :param solution_gas_to_oil_ratio_table: Pre-computed Rs(P, T) (n_p, n_t) in scf/STB.
    :param standard_oil_density: Stock-tank oil density (lbm/ft³). Used to derive
        `density_table` when absent.
    :param standard_gas_density: Stock-tank gas density (lbm/ft³). Used to derive
        `density_table` when absent.
    ...
    oil_api_gravity_table = arrays.compute_oil_api_gravity(
    ...
    if bubble_point_pressures is None:
        ...
        oil_api_gravity_1d = arrays.compute_oil_api_gravity(
        ...
        if solution_gas_to_oil_ratios is not None:
            ...
            bubble_point_pressures = arrays.compute_oil_bubble_point_pressure(
                gas_gravity=gas_gravity_1d,
                oil_api_gravity=oil_api_gravity_1d,
                temperature=temperatures,
    ...
    if bubble_point_pressures.ndim == 1:
        ...
        if solution_gas_to_oil_ratio_table is None:
            solution_gas_to_oil_ratio_table = arrays.estimate_solution_gor(
                pressure=pressure_table,
                temperature=temperature_table,
    ...
    if solution_gas_to_oil_ratio_table is None:
        solution_gas_to_oil_ratio_table = arrays.compute_gas_to_oil_ratio(
            pressure=pressure_table,
            temperature=temperature_table,
    ...
    gor_at_bubble_point_table = arrays.compute_gas_to_oil_ratio(
        pressure=bubble_point_pressure_table,
        temperature=temperature_table,
        bubble_point_pressure=bubble_point_pressure_table,
        gas_gravity=gas_gravity_table,
        oil_api_gravity=oil_api_gravity_table,
    ...
    if needs_fvf and needs_co:
        ...
        for _ in range(10):
            ...
            new_oil_compressibility = arrays.compute_oil_compressibility(
                pressure=pressure_table,
                temperature=temperature_table,
                bubble_point_pressure=bubble_point_pressure_table,
                oil_api_gravity=oil_api_gravity_table,
        ...
        compressibility_table = arrays.compute_oil_compressibility(
            pressure=pressure_table,
            temperature=temperature_table,
            bubble_point_pressure=bubble_point_pressure_table,
        ...
        if standard_oil_density is not None and standard_gas_density is not None:
            density_table = (
                (
                    ...
                    + solution_gas_to_oil_ratio_table * standard_gas_density
                )
                / formation_volume_factor_table  # type: ignore
            ).astype(dtype)
        else:
            assert formation_volume_factor_table is not None
            density_table = arrays.compute_live_oil_density(
...
def build_gas_pvt_data(
    ...
    **kwargs: typing.Any,
    ...
    :param specific_gravity_table: Pre-computed γg(P, T) (n_p, n_t).
    :param molecular_weight_table: Pre-computed Mg(P, T) (n_p, n_t) in lbm/lb-mol.
    :param solubility_in_water_table: Pre-computed Rsw(P, T, S) (n_p, n_t, n_s).
    :param standard_gas_density: Stock-tank gas density (lbm/ft³) for density derivation.
    :param standard_oil_density: Stock-tank oil density (lbm/ft³) for wet-gas density
    ...
    if specific_gravity_table is None:
        specific_gravity_table = gas_gravity_table.copy()

    if molecular_weight_table is None:
        molecular_weight_table = arrays.compute_gas_molecular_weight(
            gas_gravity=specific_gravity_table
        )

    if compressibility_factor_table is None:
        compressibility_factor_table = arrays.compute_gas_compressibility_factor(
            pressure=pressure_table,
            temperature=temperature_table,
            gas_gravity=gas_gravity_table,
        )
    ...
    if compressibility_table is None:
        compressibility_table = arrays.compute_gas_compressibility(
            pressure=pressure_table,
            temperature=temperature_table,
            gas_gravity=gas_gravity_table,
            gas_compressibility_factor=compressibility_factor_table,
...
def build_water_pvt_data(
    ...
    **kwargs: typing.Any,
    ...
    `standard_water_density` is provided; otherwise falls back to the
    ...
    :param specific_gravity_table: Pre-computed γw(P, T, S) (n_p, n_t, n_s).
    :param molecular_weight_table: Pre-computed Mw(P, T, S) (n_p, n_t, n_s).
    ...
    :param standard_water_density: Stock-tank water density (lbm/ft³) for density derivation.
    ...
    if pvt_table is not None:
        ...
        gas_fvf_table_2d = gas_tables.get("formation_volume_factor_table")
        ...
        z_factor_table_2d = arrays.compute_gas_compressibility_factor(
    ...
    gas_solubility_table_3d = arrays.compute_gas_solubility_in_water(
        pressure=pressure_table_3d,
        temperature=temperature_table_3d,
        salinity=salinity_table_3d,
        gas=gas,
    ...
    if compressibility_table is None:
        compressibility_table = typing.cast(
            ...
            arrays.compute_water_compressibility(
                ...
                temperature=temperature_table_3d,
                bubble_point_pressure=bubble_point_pressure_table,
                gas_formation_volume_factor=gas_fvf_table_3d,
                gas_solubility_in_water=gas_solubility_table_3d,
                gas_free_water_formation_volume_factor=gas_free_fvf_table_3d,
                salinity=salinity_table_3d,
            ),
        )
    ...
    if density_table is None:
        if standard_water_density is not None:
            density_table = (standard_water_density / gas_free_fvf_table_3d).astype(
                dtype
            )
        else:
            density_table = typing.cast(
    ...
    if formation_volume_factor_table is None:
        ...
        formation_volume_factor_table = arrays.compute_water_formation_volume_factor(
    ...
    if molecular_weight_table is None:
        molecular_weight_table = (18.015 + salinity_table_3d * 5.85e-5).astype(
            dtype, copy=False
        )
...
def build_pvt_dataset(
    ...
    **kwargs: typing.Any,
    ...
    :param standard_oil_density: Stock-tank oil density (lbm/ft³).
    :param standard_gas_density: Stock-tank gas density (lbm/ft³).
    :param standard_water_density: Stock-tank water density (lbm/ft³).
    :returns: `PVTDataSet`.
    """
    ...
    oil_data = (
        build_oil_pvt_data(
            ...
            solution_gas_to_oil_ratios=solution_gas_to_oil_ratios,
            gas=gas,
            standard_oil_density=standard_oil_density,
            standard_gas_density=standard_gas_density,
    ...
    water_data = (
        build_water_pvt_data(
            ...
            gas_gravity=gas_gravity,
            gas=gas,
            standard_water_density=standard_water_density,
