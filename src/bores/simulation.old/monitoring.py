...
def monitor(
    ...
    *,
    ...
    :param input: A `BlackOil`, `Run`, or an iterable that yields `ModelState`s - identical to
        the first argument of `bores.run`.
    :param config: Simulation configuration. Required when `input` is a
        `BlackOil`; optional when `input` is a `Run` (overrides
    ...
    :raises ValidationError: If `input` is a `BlackOil` and `config` is not provided.
    """
    if monitor is None:
        monitor = Monitor()

    is_generic_input = not isinstance(input, (BlackOil, Run))
    ...
    if isinstance(input, Run):
        ...
        if config is None and not is_generic_input:
            raise ValueError("Must provide `config` when `input` is a `BlackOil`.")
