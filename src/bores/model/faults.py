"""Fault definition and application API for reservoir models."""
...
class FaultRecord(Serializable):
    """
...
def apply_faults(
    model: BlackOilModel,
    *faults: FaultRecord,
    recompute_transmissibilities: bool = True,
) -> BlackOilModel:
    """
    Apply one or more `FaultRecord` definitions to a model and return a new model.
    ...
    **What changes on the returned model**:
    ...
    **What does not change**: rock, pvt, dynamic state, hysteresis, datum depth,
    ...
    :param model: The `BlackOilModel` to apply faults to.
    :param faults: One or more `FaultRecord` objects.
    :param recompute_transmissibilities: If `True` (default), transmissibilities
        are eagerly recomputed on the new model before returning. If `False`,
        they are left to be computed lazily on first access.
    :returns: A new `BlackOilModel` with the faults applied.
    ...
    new_grid = _apply_faults_to_grid(model.grid, faults, nx, ny, nz)
...
def remove_faults(
    ...
    *fault_names: str,
    ...
    """
    Remove named faults from a model and return a new model.
    ...
    :param model: The `BlackOilModel` to remove faults from.
    :param fault_names: Names of faults to remove. If none are given, all faults
        are removed.
    :param recompute_transmissibilities: If `True` (default), transmissibilities
        are eagerly recomputed on the new model before returning.
    :returns: A new `BlackOilModel` with the specified faults removed.
    :raises ValidationError: If a named fault does not exist on the model grid.
    """
    grid = model.grid
    ...
    new_grid = _remove_faults_from_grid(grid, names_to_remove)
    new_model = model.__class__(
        grid=new_grid,
        rock=model.rock,
        pvt=model.pvt,
...
def _validate_fault_bounds(
    faults: typing.Tuple[FaultRecord, ...], nx: int, ny: int, nz: int
) -> None:
    """
...
def _apply_faults_to_grid(
    grid: Grid,
    faults: typing.Tuple[FaultRecord, ...],
    nx: int,
    ny: int,
    nz: int,
) -> Grid:
